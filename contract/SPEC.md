...
| `effects` | alpha/opacity/blur policy keys | separate from RGB by design. `backgroundOpacity` is the Look's general background translucency, in (0, 1], consumed by any surface that composites over the wallpaper; `ghosttyBackgroundOpacity` is the terminal-specific refinement and outranks it for the terminal alone. A consumer resolves `.targets.<adapter>.backgroundOpacity // .effects.backgroundOpacity // 1` and the terminal inserts its own key between the two |
