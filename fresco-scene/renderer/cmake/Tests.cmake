...
if(BUILD_TESTING
   ...
   AND FRESCO_SCENE_WORKSHOP_ROOT)
    ...
    if(TARGET fresco-scene-render-smoke)
        ...
        add_test(
            NAME fresco-scene-renderer-persona-composited-text-alignment-render
            COMMAND
                "${Python3_EXECUTABLE}"
                "${CMAKE_CURRENT_SOURCE_DIR}/tests/persona_composited_text_alignment_render_test.py"
                "$<TARGET_FILE:fresco-scene-render-smoke>"
                "${FRESCO_SCENE_WORKSHOP_ROOT}"
                "${FRESCO_SCENE_ASSETS}"
        )
        set_tests_properties(
            fresco-scene-renderer-persona-composited-text-alignment-render
            PROPERTIES TIMEOUT 300
        )
        add_test(
    ...
    add_test(
        NAME fresco-scene-renderer-cursor-click-hit-test
        COMMAND
            "${Python3_EXECUTABLE}"
            "${CMAKE_CURRENT_SOURCE_DIR}/tests/cursor_click_hit_test_test.py"
            "$<TARGET_FILE:fresco-scene>"
            "${FRESCO_SCENE_WORKSHOP_ROOT}"
            "${FRESCO_SCENE_ASSETS}"
            "${FRESCO_SCENE_RENDER_BACKEND}"
    ...
    add_test(
        NAME fresco-scene-renderer-scene-script-angle-units
        COMMAND
            "${Python3_EXECUTABLE}"
            "${CMAKE_CURRENT_SOURCE_DIR}/tests/scene_script_angle_units_test.py"
            "$<TARGET_FILE:fresco-scene>"
            "${FRESCO_SCENE_WORKSHOP_ROOT}"
            "${FRESCO_SCENE_ASSETS}"
            "${FRESCO_SCENE_RENDER_BACKEND}"
