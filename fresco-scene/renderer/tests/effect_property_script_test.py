...
assert ready["genericPropertyScriptChanges"] == 54, ready
...
assert first_change["genericPropertyScriptChanges"] == 59, first_change
assert paused["paused"] is True, paused
assert paused["genericPropertyScriptUpdates"] == 392, paused
assert paused["genericPropertyScriptChanges"] == 59, paused
...
assert resumed["genericPropertyScriptChanges"] == 64, resumed
...
for event in (reloaded, reloaded_metrics):
    ...
    assert event["genericPropertyScriptChanges"] == 54, event
